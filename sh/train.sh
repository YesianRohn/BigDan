...
for ((i=0;i<5;i+=1))
do
python main.py --data-path ${data_path} --dataset_list ${dataset_list[i]} --model ${model} --output_dir ${output_dir_task1[i]} --batch-size ${bach_size} --epochs ${epochs} --lr ${lr} --weight-decay ${weight_decay} > ${stdout_log}
done
